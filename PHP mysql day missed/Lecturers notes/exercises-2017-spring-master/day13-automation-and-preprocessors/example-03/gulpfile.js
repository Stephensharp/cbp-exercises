...
gulp.task('develop', ['csscompile'], function(){
	browserSync.init({ // initalize Browsersync
		// set what server will be proxied (your local url with port)
		proxy: {
			target: 'http://www.cbp-exercises.local:8080'
		},
		host: 'www.cbp-exercises.local', // this tells where the browserSync server will run…
		port: 3000, // …and on what port
		files: ['**/*.html', '**/*.php'] // watch for changes all files named anything.html and anything.php in any subfolder
	});
